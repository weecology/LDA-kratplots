...
# 
# rmpe = data.frame(censusdate = pe$censusdate,treatment=pe$treatment, mean_abund = pe$mean_abund+rm$mean_abund+pf$mean_abund+pm$mean_abund,pp$mean_abund+ba$mean_abund)
...
# 
# # focus on RM and PE
# rm = rm %>% mutate(year=as.numeric(format(censusdate,'%Y')))
