...
chpt_histogram_plot = function(changepoint, rodent_data, binwidth) {
  ...
  cps_years <- select(rodent_data, date, timestep) %>%
  ...
  histoplot = ggplot(data = cps_gathered, aes(x=date_numeric)) +
    geom_histogram(data=subset(cps_gathered,cpt=='V1'), aes(y=..count../sum(..count..)), binwidth=binwidth, fill='black', alpha=.2) +
    geom_histogram(data=subset(cps_gathered,cpt=='V2'), aes(y=..count../sum(..count..)), binwidth=binwidth, fill='black', alpha=.6) +
    geom_histogram(data=subset(cps_gathered,cpt=='V3'), aes(y=..count../sum(..count..)), binwidth=binwidth, fill='black', alpha=.4) +
    geom_histogram(data=subset(cps_gathered,cpt=='V4'), aes(y=..count../sum(..count..)), binwidth=binwidth, fill='black', alpha=1) +
    labs(x='',y='') +
