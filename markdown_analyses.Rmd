...
It seems to be split between k = 3 and k = 4. 

Looking at the communities for the best seeds for k = 3 and k = 4: 
...
seeds_4topics = best_ntopic %>% 
  ...
  head(100) %>% 
  pull(SEED)
best_seed_4 = calculate_LDA_distance(dat,seeds_4topics, k =4)
...
seeds_3topics = best_ntopic  %>% 
  ...
  head(100) %>% 
  pull(SEED)
best_seed_3 = calculate_LDA_distance(dat,seeds_3topics, k =3)

```

### Run LDA and changepoint models

Save these models rather than keeping them all in memory and re-running...

#### Eight plots starting in 1988, 3 topics
```{r}

# LDA model

SEED_3 = unlist(best_seed_3)[1]

ntopics = 3

ldamodel3 = LDA(dat,ntopics, control = list(seed = SEED_3),method='VEM')
